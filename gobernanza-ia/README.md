...
|-------|---------|-------------|
| Triaje | `/triaje` | Clasifica un sistema de IA según su nivel de riesgo conforme a un marco de referencia por principios (inaceptable, alto, limitado, mínimo) |
| Evaluación | `/evaluacion` | Guía la evaluación de impacto de un sistema de IA de alto riesgo |
| Proveedor | `/proveedor` | Analiza las obligaciones y riesgos asociados a un proveedor de IA |
| Gap analysis | `/gap` | Identifica brechas de gobernanza interna de IA frente a principios y marcos de referencia, y a la normativa panameña conexa |

## Primeros pasos

1. Ejecuta la entrevista inicial del proyecto para configurar tu perfil y preferencias.
2. Prueba `/triaje` con la descripción de un sistema de IA para clasificar su nivel de riesgo.
3. Usa `/proveedor` para evaluar las condiciones contractuales y técnicas de un proveedor de IA.
4. Ejecuta `/gap` para obtener un mapa de gobernanza frente a tu marco interno de referencia.
...
- **No realiza auditoría técnica de algoritmos**: no ejecuta pruebas de sesgo, explicabilidad ni robustez técnica de modelos.
- **No certifica sistemas de IA**: no emite certificaciones de conformidad ni sellos de calidad.
- **No desarrolla documentación técnica de sistemas**: no genera la documentación técnica detallada de los modelos.
- **No sustituye el criterio profesional** del abogado: las recomendaciones deben ser validadas por un profesional cualificado.
